...
from .permissions import IsMovieOwner, IsMovieOwnerAndCreator, IsAdminOrStaff
...
class MovieCreateView(generics.CreateAPIView):
    ...
    permission_classes = [permissions.IsAuthenticated, IsMovieOwner]
...
class MovieUpdateView(generics.UpdateAPIView):
    queryset = Movie.objects.filter(is_deleted=False)
    serializer_class = MovieCreateUpdateSerializer
    permission_classes = [permissions.IsAuthenticated, IsMovieOwnerAndCreator]
    lookup_field = 'slug'
...
class MovieRestoreView(generics.UpdateAPIView):
    ...
    def update(self, request, *args, **kwargs):
        ...
        if not request.user.is_superuser and movie.created_by != request.user:
            return Response({"detail": "You do not have permission to restore this movie."}, status=403)
...
class ReviewListCreateView(generics.ListCreateAPIView):
    ...
    def get_queryset(self):
        return Review.objects.filter(movie__slug=self.kwargs['slug'])
